...
namespace hope {
    ...
    inline
    void assert_load_valid(const hope::json::ParseResult& ok) {
        if (!ok) {
    ...
    std::string extract_exception_with_name(std::string_view name){
        return std::string("hope::extract, the object with name [") +
                name.data() + "] ";
    }

    inline
    void assert_object_not_null(hope::json::Value& obj, std::string_view name){
        if(obj.IsNull()) {
            throw std::runtime_error(
    ...
    void assert_type_valid(hope::json::Value& obj, std::string_view name) {
        ...
        if constexpr (hope::is_vector_v<TValue>){
            validate_obj(&hope::json::Value::IsArray, "Array");
        }
        if constexpr (is_object_v<TValue>) {
            validate_obj(&hope::json::Value::IsObject, "Object");
        }
        if constexpr (is_inbuild_v<TValue>){
            const hope::type_value_map map(
                hope::tv<raw_string_t>(std::pair(&hope::json::Value::IsString, "String")),
                hope::tv<raw_int_t>(std::pair(&hope::json::Value::IsInt, "Int")),
                hope::tv<raw_bool_t>(std::pair(&hope::json::Value::IsBool, "Bool")),
                hope::tv<raw_real_t>(std::pair(&hope::json::Value::IsDouble, "Real"))
            );
