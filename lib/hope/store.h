...
namespace hope {
    ...
    public:
        ...
        void execute(hope::json::Document& document) {
    ...
    private:
        constexpr auto setter_map() const {
            return hope::type_value_map(
                    hope::tv<raw_int_t>(&hope::json::Value::SetInt),
                    hope::tv<raw_bool_t>(&hope::json::Value::SetBool),
                    hope::tv<raw_real_t>(&hope::json::Value::SetDouble)
            );
        }

        template<typename TObject>
        void store_impl(const raw_array_t<TObject>& value, hope::json::Value& object) {
            object.SetArray();
            for(auto&& obj : value) {
        ...
        template<typename TObject>
        void store_impl(const TObject& value, hope::json::Value& json_val) {
            if constexpr (is_inbuild_v<TObject>) {
                ...
                tuple.for_each([&](auto&& child){
                    ...
                    static_assert(is_object_v<child_t>,
                        "HOPE-JSON::store: All the types of the user defined structure"
